...
The docker image will run controlroom-bot with a SQLite database and
...
* Choose whether you want to use SQLite or Postgres as your database backend.
  ...
  ```
  database: "postgres://username:password@postgres/controlroom-bot?sslmode=disable"
  ```
...
```
docker-compose up controlroom-bot
```
...
```
docker-compose up -d controlroom-bot
```

(Logs can later be accessed with the `docker logs` command).

This will use the `latest` tag from
[Docker Hub](https://hub.docker.com/somebody/controlroom-bot).
...
```
docker-compose pull controlroom-bot
```
...
[controlroom-bot.service](controlroom-bot.service). The service uses
`docker-compose` to start and stop the bot.

Copy the file to `/etc/systemd/system/controlroom-bot.service` and edit to
match your setup. You can then start the bot with:

```
systemctl start controlroom-bot
```

and stop it with:

```
systemctl stop controlroom-bot
```

To run the bot on system startup:

```
systemctl enable controlroom-bot
```
...
```
docker build -t somebody/controlroom-bot:latest -f docker/Dockerfile .
```
